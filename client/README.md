...
会在服务器存储和请求Cookie时在游戏日志中打印其内容（Payload使用Base64编码显示）。  
...
忽略列表是一个Identifier列表，存储在客户端的模组配置文件中，也可以通过命令管理。在收到服务端存储Cookie的请求时，会检查其Key是否在忽略列表中，如果被忽略则不会实际写入serverCookie中。
